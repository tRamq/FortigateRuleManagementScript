...
class Fortigate:
    ...
    def AddFwPolicy(self, srcintf='port3', dstintf='port4', srcaddr='all', dstaddr='all', service='HTTPS', action='accept', schedule='always',status='enable', name='new policy'):
        """]'
        Create a fw policy.
    ...
    def AddFwPolicyIdempotent(self, srcintf='any', dstintf='any', srcaddr='all', dstaddr='all', service='ALL', action='accept', schedule='always', status='enable', name=''):
        """
        ...
        service= str(service)
        action= str(action)
        print("gonderilen parametreler  ->>",srcintf,dstintf,srcaddr,dstaddr,service,action,schedule,nat,poolname,ippool,status,comments,traffic_shaper,traffic_shaper_reverse)
        objects =  [['srcintf',srcintf],['dstintf',dstintf],['srcaddr',srcaddr],['dstaddr',dstaddr],['service',service],['action',action],['schedule',schedule],['status',status],['name',name]] 
        if not (self.Exists('cmdb/firewall/policy/', objects)):
            #object does not exist, create it
            print ('AddFwPolicyIdempotent: object does not exists')
            return self.AddFwPolicy(srcintf, dstintf, srcaddr, dstaddr, service, action, schedule, status, name)
        else: 
            #object already Exists
